...
VALUE rb_Color_Initialize(int argc, VALUE* argv, VALUE self)
{
    ...
    if (RTEST(red))   color->r = (uint8_t)normalize_long(RB_NUM2LONG(red),   0, 255);
    if (RTEST(green)) color->g = (uint8_t)normalize_long(RB_NUM2LONG(green), 0, 255);
    if (RTEST(blue))  color->b = (uint8_t)normalize_long(RB_NUM2LONG(blue),  0, 255);
    if (RTEST(alpha)) color->a = (uint8_t)normalize_long(RB_NUM2LONG(alpha), 0, 255);
    return self;
}
...
{
    if (!rb_obj_is_kind_of(other, rb_cColor)) return Qfalse;
